...
func testPhpAppConfig(t *testing.T, when spec.G, it spec.S) {
	...
	it("generates an httpd.conf from the template", func() {
		...
		Expect(result).To(ContainSubstring(`DocumentRoot "${HOME}/htdocs"`))
		Expect(result).To(ContainSubstring(`<Directory "${HOME}/htdocs">`))
		...
		Expect(result).To(ContainSubstring(`Define fcgi-listener fcgi://127.0.0.1:9000${HOME}/htdocs`))
		Expect(result).To(ContainSubstring(`<Proxy "${fcgi-listener}">`))
		Expect(result).To(ContainSubstring(`ProxySet disablereuse=On retry=0`))
		Expect(result).To(ContainSubstring(`<Directory "${HOME}/htdocs">`))
		Expect(result).To(ContainSubstring(`SetHandler proxy:${fcgi-listener}`))
